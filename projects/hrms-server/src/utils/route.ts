...
export function curd<X extends zodAny, Y extends z.ZodTypeAny, Z extends zodAny>(
  tableInfo: DrizzleTableInfo<X, Y, Z>,
) {
  return {
    ...
    create: t.procedure.input(tableInfo.insertValidation).mutation(async ({ input, ctx }) => {
      return await ctx.db.insert(tableInfo.table).values(input).returning();
    }),
